...
func init() {
	RegisterCommand(Command{
		...
		Short: "Starts the Pggat process in the background and then returns",
		Long: `
Starts the Pggat process, optionally bootstrapped with an initial config file.
...
the KEY=VALUE format will be loaded into the Pggat process.

On Windows, the spawned child process will remain attached to the terminal, so
closing the window will forcefully stop Pggat; to avoid forgetting this, try
...
`,
	...
	RegisterCommand(Command{
		...
		Short: `Starts the Pggat process and blocks indefinitely`,
		Long: `
Starts the Pggat process, optionally bootstrapped with an initial config file,
and blocks indefinitely until the server is stopped; i.e. runs Pggat in
"daemon" mode (foreground).

If a config file is specified, it will be applied immediately after the process
is running. If the config file is not in Pggat's native JSON format, you can
specify an adapter with --adapter to adapt the given config file to
Pggat's native format. The config adapter must be a registered module. Any
...
"Gatfile" and the caddyfile config adapter is plugged in (default), then
that file will be loaded and used to configure Pggat, even without any command
line flags.

If --envfile is specified, an environment file with environment variables in
the KEY=VALUE format will be loaded into the Pggat process.

If --environ is specified, the environment as seen by the Pggat process will
...
`,
	...
	RegisterCommand(Command{
		...
		Short: "Gracefully stops a started Pggat process",
		Long: `
Stops the background Pggat process as gracefully as possible.
...
`,
	...
	RegisterCommand(Command{
		Name:  "reload",
		Usage: "--config <path> [--adapter <name>] [--address <interface>]",
		Short: "Changes the config of the running Pggat instance",
		Long: `
Gives the running Pggat instance a new configuration. This has the same effect
...
`,
	...
	RegisterCommand(Command{
		...
		Long: `
Prints the version of this Pggat binary.

Version information must be embedded into the binary at compile-time in
order for Pggat to display anything useful with this command. If Pggat
is built from within a version control repository, the Go command will
embed the revision hash if available. However, if Pggat is built in the
...
`,
	...
	RegisterCommand(Command{
		...
		Short: "Lists the installed Pggat modules",
		CobraFunc: func(cmd *cobra.Command) {
	...
	RegisterCommand(Command{
		...
		Long: `
Prints the environment as seen by this Pggat process.

The environment includes variables set in the system. If your Pggat
...
Note that environments may be different depending on how you run Pggat.
Environments for Pggat instances started by service managers such as
...
`,
	...
	RegisterCommand(Command{
		...
		Short: "Adapts a configuration to Pggat's native JSON",
		Long: `
Adapts a configuration to Pggat's native JSON format and writes the
...
`,
	...
	RegisterCommand(Command{
		Name:  "storage",
		Short: "Commands for working with Pggat's storage (EXPERIMENTAL)",
		Long: `
Allows exporting and importing Pggat's storage contents. The two commands can be
combined in a pipeline to transfer directly from one storage to another:

$ caddy storage export --config Gatfile.old --output - |
> caddy storage import --config Gatfile.new --input -
...
`,
	...
	RegisterCommand(Command{
		...
		Short: "Formats a Gatfile",
		Long: `
Formats the Gatfile by adding proper indentation and spaces to improve
...
`,
	...
	RegisterCommand(Command{
		Name:  "upgrade",
		Short: "Upgrade Pggat (EXPERIMENTAL)",
		Long: `
Downloads an updated Pggat binary with the same modules/plugins at the
...
`,
	...
	RegisterCommand(Command{
		Name:  "add-package",
		Usage: "<packages...>",
		Short: "Adds Pggat packages (EXPERIMENTAL)",
		Long: `
Downloads an updated Pggat binary with the specified packages (module/plugin)
...
`,
	...
	RegisterCommand(Command{
		...
		Usage: "<packages...>",
		Short: "Removes Pggat packages (EXPERIMENTAL)",
		Long: `
Downloads an updated Pggat binaries without the specified packages (module/plugin).
...
`,
	...
	RegisterCommand(Command{
		...
		Short: "Generates the manual pages for Pggat commands",
		Long: `
Generates the manual pages for Pggat commands into the designated directory
